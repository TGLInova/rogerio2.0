<x-ui::section x-data="{ animate: false }" x-intersect.half:enter="animate = true" x-intersect.half:leave="animate = false">
    <x-ui::container class="grid lg:grid-cols-2 gap-16 items-center">
        <div class="space-y-6 flex flex-col lg:items-start" x-bind:class="{'*:animate-fade-up' : animate }">
            ...
            <ul class="[&>li]:flex [&>li>svg]:text-primary-400 [&>li]:items-center [&>li]:gap-2 space-y-6 animate-delay-200">
        ...
        <img src="{{ asset('static/img/materiais_exclusivos.webp') }}" x-bind:class="{ 'animate-delay-400 animate-fade': animate }" class="max-w-96 mx-auto">
    </x-ui::container>
</x-ui::section>
